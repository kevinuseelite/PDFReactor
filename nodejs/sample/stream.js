...
fs.readFile('../../resources/contentNodeJs.html', 'utf8', async (error, content) => {
    ...
    try {
        ...
        const documentId = await pdfReactor.convertAsync(config, connectionSettings);
        let progress = null;

        do {
            await sleep(0.5);
            progress = await pdfReactor.getProgress(documentId, connectionSettings);
        } while (!progress.finished);

        const file2 = fs.createWriteStream('stream-async.pdf', 'binary');
        await pdfReactor.getDocumentAsBinary(documentId, file2, connectionSettings);
